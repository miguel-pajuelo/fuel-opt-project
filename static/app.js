    ...
    const BRAND_LOGO_KEYS = Object.keys(BRAND_LOGOS)
      .sort((a, b) => normalizeBrandLogoKey(b).length - normalizeBrandLogoKey(a).length);

    function brandLogoCandidateValues(brand) {
      if (!brand) return [];
    ...
    function brandLogoFor(brand) {
      const values = brandLogoCandidateValues(brand);
      ...
      for (const value of values) {
        ...
        for (const key of BRAND_LOGO_KEYS) {
          const normalizedKey = normalizeBrandLogoKey(key);
          if (
    ...
    function stationLogoHtml(station, className = 'result-station-logo') {
      const logoSrc = brandLogoFor(station);
