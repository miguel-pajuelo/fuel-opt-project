...
def test_catalog_refresh_requires_admin_token() -> None:
    previous_settings = api_main.settings
    try:
        api_main.settings = Settings(admin_token=None)
        try:
            api_main._require_catalog_refresh_auth()
        except HTTPException as exc:
            _assert(exc.status_code == 403, f"Unexpected status without configured token: {exc.status_code}")
        else:
            raise AssertionError("Expected HTTPException when refresh token is not configured.")

        api_main.settings = Settings(admin_token="expected-token")
        try:
            api_main._require_catalog_refresh_auth(x_fuelopt_admin_token="wrong-token")
        except HTTPException as exc:
            _assert(exc.status_code == 401, f"Unexpected status with wrong token: {exc.status_code}")
        else:
            raise AssertionError("Expected HTTPException with wrong refresh token.")

        api_main._require_catalog_refresh_auth(x_fuelopt_admin_token="expected-token")
        api_main._require_catalog_refresh_auth(authorization="Bearer expected-token")
    finally:
        api_main.settings = previous_settings


def test_api_docs_hidden_by_default() -> None:
    public_paths = {getattr(route, "path", "") for route in api_main.app.routes}
    _assert("/docs" not in public_paths, "FastAPI docs should be hidden by default.")
    _assert("/redoc" not in public_paths, "FastAPI redoc should be hidden by default.")
    _assert("/openapi.json" not in public_paths, "OpenAPI schema should be hidden by default.")
...
def run() -> None:
    ...
    test_catalog_refresh_requires_admin_token()
    test_api_docs_hidden_by_default()
