...
def test_map_has_user_location_control() -> None:
    ...
    _assert("USER_LOCATION_ZOOM_BOOST" in js, "User location zoom boost constant missing.")
    _assert("USER_LOCATION_MIN_ZOOM" in js, "User location minimum zoom constant missing.")
    _assert("function userLocationTargetZoom" in js, "User location target zoom helper missing.")
    _assert("map.getZoom() + USER_LOCATION_ZOOM_BOOST" in js, "User location should zoom in from the current map zoom.")
    _assert("map.getMaxZoom" in js, "User location zoom should respect the map maximum zoom.")
    _assert("map.setView(latLng, targetZoom" in js, "Map should center on geolocation coordinates with an explicit boosted zoom.")
